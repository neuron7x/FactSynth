...
> Контрактні тести покладаються на бібліотеку `requests`; якщо її немає,
> `pytest.importorskip("requests")` пропустить ці тести.
> Завантажувач політик якості використовує `PyYAML`; вона входить до базових залежностей,
> тож `pip install -r requirements.lock` або `pip install -e .[dev]` встановить її автоматично.
